...
interface HyperTextProps extends Omit<MotionProps, "children"> {
  ...
  /** Local change: render letters in upper case (the stock look). */
  uppercase?: boolean
}
...
export function HyperText({
  ...
  return (
    <MotionComponent
      ref={elementRef}
      className={cn(uppercase && "overflow-hidden py-2 text-4xl font-bold", className)}
      ...
      <AnimatePresence>
        {displayText.map((letter, index) => (
          <motion.span
            key={index}
            aria-hidden
            className={cn(uppercase && "font-mono", letter === " " ? "w-3" : "")}
          >
